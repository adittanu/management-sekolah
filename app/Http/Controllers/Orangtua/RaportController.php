...
class RaportController extends Controller
{
    public function exportPdf(Request $request)
    {
        ...
        $isChild = $user->children()->where('users.id', $validated['student_id'])->exists();
        if (! $isChild) {
            return redirect()->back()->with('error', 'Anda tidak memiliki akses ke raport siswa ini.');
        }
    ...
    public function index(Request $request)
    {
        ...
        if ($studentId && $children->pluck('id')->contains($studentId)) {
            $student = User::find($studentId);
            $classroom = $student->classrooms()->first();

            if ($classroom) {
                $raportData = $this->buildRaportData($studentId, $classroom->id, $academicYear, $semester, $reportType);
            }
        }
        ...
        return Inertia::render('Orangtua/Raport', [
            'children' => $children,
