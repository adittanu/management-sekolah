...
{
    /**
     * Input nilai - show form to input grades per class & subject.
     */
    public function index(Request $request)
    {
        ...
        if ($isTeacher) {
            $myClassroomIds = \App\Models\Schedule::where('teacher_id', $user->id)->pluck('classroom_id')->unique();
            $classrooms = Classroom::whereIn('id', $myClassroomIds)->get();
            $subjects = Subject::whereHas('teachers', fn ($q) => $q->where('users.id', $user->id))->with('teachers')->get();
        } else {
    ...
    /**
     * Save grades for all students in a class.
     */
    public function store(Request $request)
    {
        ...
        if (! $teacherId) {
            $subject = Subject::with('teachers')->find($validated['subject_id']);
            $teacherId = $subject->teachers->first()?->id;
        }
        ...
        return redirect()->back()->with([
            'auto_generate_result' => [
                'auto_generate_saved' => $saved,
                'auto_generate_stats' => ['total_lessons' => $saved, 'scheduled' => $saved, 'fill_rate' => 100],
            ],
        ]);
    }
    ...
    /**
     * Raport view - show raport for a student.
     */
    public function raport(Request $request)
    {
        ...
        if ($isTeacher) {
            ...
            $classrooms = $homeroomClass ? collect([$homeroomClass]) : collect();
        } else {
            $classrooms = Classroom::all();
        }
        ...
        return Inertia::render('Admin/Rapor/Raport', [
            'classrooms' => $classrooms,
            'students' => $students,
    ...
    /**
     * Build raport data for a specific student.
     */
