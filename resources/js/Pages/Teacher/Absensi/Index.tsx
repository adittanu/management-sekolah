...
// Types
type AttendanceStatus = 'Hadir' | 'Sakit' | 'Izin' | 'Alpha' | 'Tidak Mengabsen';
...
export default function TeacherAbsensiIndex({ history, schedules, stats }: Props) {
    ...
    const normalizeStatus = (s: string): AttendanceStatus => {
        ...
        if (lower === 'tidak_mengabsen') return 'Tidak Mengabsen';
    ...
    const isAutoGenerated = (item: JournalModel) =>
        normalizeStatus(item.teacher_status ?? '') === 'Tidak Mengabsen';
    ...
    const getStatusColor = (status: AttendanceStatus) => {
        switch (status) {
            ...
            case 'Tidak Mengabsen': return 'bg-slate-100 text-slate-600 hover:bg-slate-200 border-slate-300';
                        ...
                        <Card className="border-none shadow-sm bg-white overflow-hidden">
                            <CardHeader className="px-6 py-4 border-b bg-slate-50/50">
                                ...
                                <div className="flex flex-wrap items-center gap-2 pt-2">
                                    ...
                                    {(['Semua', 'Hadir', 'Sakit', 'Izin', 'Alpha', 'Tidak Mengabsen'] as const).map((status) => (
                                        <Button
                                            key={status}
                                            type="button"
                                            variant="ghost"
                                ...
                                {history.data.length === 0 ? (
                                    ...
                                    <div className="divide-y divide-slate-100">
                                        {filteredHistory.map((item) => (
                                            <div key={item.id} className={`p-6 hover:bg-slate-50 transition-colors ${isAutoGenerated(item) ? 'bg-slate-50/70' : ''}`}>
                                                {isAutoGenerated(item) && (
                                                    <div className="mb-3 flex items-center gap-2 text-xs text-slate-500 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2">
                                                        <Clock className="w-3.5 h-3.5 shrink-0" />
                                                        <span>Absensi ini diisi otomatis oleh sistem karena Anda tidak melakukan presensi setelah jam pelajaran berakhir.</span>
                                                    </div>
                                                )}
