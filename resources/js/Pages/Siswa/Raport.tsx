...
interface RaportData {
    student: { id: number; name: string; email?: string; identity_number?: string; address?: string };
...
export default function SiswaRaport({ raportData, raportHistory = [], filters }: Props) {
    ...
    const handleFilterChange = (overrides?: { semester?: string; academicYear?: string; reportType?: string }) => {
        setIsFiltering(true);
        router.get(route('siswa.rapor'), {
    ...
    const handleDownloadPdf = () => {
        ...
        window.location.href = route('siswa.rapor.export-pdf') + '?' + params.toString();
            ...
            <div className="space-y-6 max-w-7xl mx-auto pb-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 p-6 rounded-2xl shadow-sm">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Raport Hasil Belajar Saya</h2>
                            <p className="text-slate-600 text-sm mt-1">Pantau dan unduh lembar penilaian hasil belajar semesteran Anda di sekolah.</p>
                        </div>
                    </div>
                </div>
                        ...
                        {isFiltering ? (
                            ...
                            <Card className="border-slate-200 shadow-sm overflow-hidden">
                                <div className="bg-slate-50 border-b border-slate-100 p-6">
                                    ...
                                    <p className="text-xs text-slate-500 mt-0.5 font-medium">Pilih periode tahun ajaran dan semester di sidebar kiri untuk memuat lembar penilaian Anda.</p>
                                </div>
                                <CardContent className="pt-10 pb-16 flex flex-col items-center justify-center text-center max-w-md mx-auto">
                                    ...
                                    <p className="text-slate-500 text-sm mt-2 mb-6">
                                        Pilih periode di sidebar kiri untuk memuat lembar capaian prestasi belajar, kehadiran, dan catatan perkembangan kepribadian Anda.
                                    </p>
                                </CardContent>
                            </Card>
                        )}
