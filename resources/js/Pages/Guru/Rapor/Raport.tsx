...
export default function Raport({ myClassrooms, students, raportData, raportHistory = [], filters }: Props) {
    ...
    const handleFilterChange = (overrides?: { classroomId?: string; studentId?: string; semester?: string; academicYear?: string; reportType?: string }) => {
        setIsFiltering(true);
        router.get(route('guru.rapor.view'), {
    ...
    const handleDownloadPdf = () => {
        ...
        window.location.href = route('guru.rapor.export-pdf') + '?' + params.toString();
    ...
    return (
        <TeacherLayout title="Raport Siswa">
            <div className="space-y-6 max-w-7xl mx-auto pb-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 p-6 rounded-2xl shadow-sm">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Raport Hasil Belajar</h2>
                            <p className="text-slate-600 text-sm mt-1">Lihat, cetak, dan ekspor lembar rapor digital siswa sesuai dengan standar kurikulum sekolah.</p>
                        </div>
                    </div>
                </div>
                        ...
                        <Card className="border-slate-200 shadow-sm">
                            <CardHeader className="pb-3 border-b border-slate-100 bg-slate-50/50">
                                ...
                                <CardDescription className="text-xs">
                                    Pilih kelas dan siswa untuk memuat rapor digital.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="pt-4 space-y-4">
                                <div className="space-y-1.5">
                                    <Label className="text-slate-600 text-xs font-semibold">Kelas</Label>
                                    <Select value={classroomId} onValueChange={(v) => { setClassroomId(v); setStudentId(''); setTimeout(() => handleFilterChange({ classroomId: v, studentId: '' }), 100); }}>
                                ...
                                <div className="space-y-1.5">
                                    <Label className="text-slate-600 text-xs font-semibold">Siswa</Label>
                                    <Select value={studentId} onValueChange={(v) => { setStudentId(v); setTimeout(() => handleFilterChange({ studentId: v }), 100); }}>
                                        <SelectTrigger className="border-slate-200"><SelectValue placeholder="Pilih Siswa" /></SelectTrigger>
                                        <SelectContent>
                        ...
                        {isFiltering ? (
                            <Card className="border-slate-200 shadow-sm">
                                <CardContent className="flex flex-col items-center justify-center py-24 text-center">
                                    ...
                                    <p className="text-slate-500 font-medium">Memuat data lembar rapor...</p>
                                </CardContent>
                            </Card>
                        ) : raportData ? (
                            ...
                            <Card className="border-slate-200 shadow-sm overflow-hidden">
                                <div className="bg-slate-50 border-b border-slate-100 p-6">
                                    <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                                        ...
                                        Langkah Pembuka: Filter Rapor
                                    </h3>
                                    <p className="text-xs text-slate-500 mt-0.5 font-medium">Pilih kelas, siswa, dan periode di sidebar kiri untuk melihat lembar rapor digital.</p>
                                </div>
                                <CardContent className="pt-10 pb-16 flex flex-col items-center justify-center text-center max-w-md mx-auto">
                                    ...
                                    <p className="text-slate-500 text-sm mt-2 mb-6">
                                        Pilih siswa dari kelas ajar Anda di sidebar sebelah kiri untuk melihat seluruh ringkasan prestasi belajar, kehadiran, dan catatan perkembangan kepribadian.
                                    </p>
                                </CardContent>
                            </Card>
                        )}
